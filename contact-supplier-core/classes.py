...
class MessageHandler:
    ...
    def get_messages(self, contact_num):
        retrieving_url = constants.get_message_url.format(
            constants.WATI_SERVER_URL, contact_num)
    ...
    def send_template(self, contact_num, template_name, broadcast_name=""):
        ...
        broadcast_name = template_name + "_" + \
            contact_num if not len(broadcast_name) else broadcast_name
        template_send_url = constants.send_template_payload.format(
            constants.WATI_SERVER_URL, contact_num)
...
class Contact:
    ...
    def send_patch(self, payload={}):
        ...
        response = requests.request(
            "PATCH", url, headers=constants.make_patch_url.format(self.phone), data=payload)
        print(response.text)
...
class Supplier:
    ...
    def __init__(self, name, pincode, contact_num, supplier_type, chosen_language):
        self.name = name
        self.pincode = pincode if pincode else self.pincode

        if ',' in contact_num:
            contact_num = contact_num.split(',')[0]

        self.contact_num = '91' + \
            contact_num if len(contact_num) == 10 else contact_num
        self.supplier_type = supplier_type if supplier_type else self.supplier_type
        self.chosen_language = chosen_language if chosen_language else self.chosen_language
    ...
    def send_template(self):
        ...
        url = constants.send_template_url.format(
            constants.WATI_SERVER_URL, self.contact_num)
        headers = constants.send_template_headers

        print(url)
        lang_template = constants.send_template_language_mapping.get(
            self.chosen_language, 'EN')
        try:

            payload = constants.send_template_payload.format(
                lang_template, lang_template+'_'+datetime.date.today().isoformat(), self.supplier_type, self.name)
            print(payload)
            response = {}

            # response = requests.request("POST", url, headers=headers, data=payload)
            # print(response)
            # curly = curlify.to_curl(response.request)
            # print(curly)
            # print(response.text)
            # j = json.loads(response.text)
            # print(j)
...
class SupplierList:
    ...
    def get_suppliers(self, num_suppliers=10, page_num=1):
        ...
        response = requests.request("GET",
                                    ...
                                    data={'pageSize': '{0}'.format(
                                        num_suppliers), 'pageNumber': '{0}'.format(page_num)},
    ...
    def get_and_parse(self, num_suppliers=10, page_num=1):
        while page_num < constants.supplier_list_page_num_max:
            ...
            supplier_json = self.get_suppliers(
                num_suppliers=num_suppliers, page_num=page_num)
            try:
    ...
    def get_introbot_repliers(self):
        url = constants.get_introbot_repliers
        response = requests.request("GET", url, headers={}, data={})
        print(response)
        j = json.loads(response.text)
        self.introbot_json_data = j['data']

        allowed_keys = set()
        for supplier_data in self.introbot_json_data:
            for key in supplier_data.keys():
                allowed_keys.add(key)

        df = pd.DataFrame(columns=list(allowed_keys))

        for supplier_data in self.introbot_json_data:
            df = df.append(supplier_data, ignore_index=True)
        df = df.applymap(str)
        df.to_csv("/home/konverge/Desktop/Work/wati/boarded_contacts_1105.csv")
...
if __name__ == "__main__":
    ...
    # cont = Contact()
    # cont.get_introbot_repliers()
    k = Supplier(name='Kalpit', contact_num='9730117966',
                 chosen_language='EN', pincode='440025', supplier_type='oxygen')
    k.send_template()
