import os

#export WATI_SERVER_URL=live-server-940.wati.io
#export WATI_SERVER_RETRIEVE_TOKEN=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..ncsrllceUk8E68o3JJfL-oAPBre1btSjAaIj426JL2I
#export WATI_SERVER_SEND_TOKEN=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..NgKDMPIHKDnSPUu6Q9j0zecVslQYYtkxw42qtmEVU_E

WATI_SERVER_URL = os.environ.get('WATI_SERVER_URL')
WATI_SERVER_RETRIEVE_TOKEN = os.environ.get('WATI_SERVER_RETRIEVE_TOKEN')
WATI_SERVER_SEND_TOKEN = os.environ.get('WATI_SERVER_SEND_TOKEN')
MIN_POOL_SIZE = 100 # Add req value as per req no of rows

get_message_url = "https://{0}/api/v1/getMessages/{1}"
get_message_payload = {'pageSize': '100', 'pageNumber': '1'}
get_message_files = []
get_message_headers = {'Authorization': 'Bearer {}'.format(WATI_SERVER_RETRIEVE_TOKEN),  'Cookie': '__cfduid=dc533c9ba4cb8c277f79928e3978a52251619958624'}


get_contacts_url = "https://{0}/api/v1/getContacts".format(WATI_SERVER_URL)
get_contacts_files = []

get_contact_payload={'pageSize': '{0}','pageNumber': '{0}'}
get_contact_headers = {'Authorization': 'Bearer {}'.format(WATI_SERVER_RETRIEVE_TOKEN),  'Cookie': '__cfduid=dc533c9ba4cb8c277f79928e3978a52251619958624'}

get_introbot_contacts = "http://api.covidcitizens.org/api/v1/leadbyquerybypage?phone={0}"
get_introbot_repliers = 'http://api.covidcitizens.org/api/v1/newleadbyquery?status_whatsapp=REPLIED'

make_patch_url = "http://api.covidcitizens.org/api/v1/updatelead?phone={0}&contact_type=Mobile"
make_patch_headers = {
  'Content-Type': 'application/json'
}
supplier_list_page_num_max = 3


send_template_url = "https://{0}/api/v1/sendTemplateMessage/{1}"

send_template_payload="{{\"template_name\": \"{0}\",\"broadcast_name\": \"{1}\",'parameters': '[{{\"name\":\"item_name\",\"value\":\"{2}\"}},{{\"name\":\"name\", \"value\":\"{3}\"}}]'}}"

#'{{"template_name": "{0}","broadcast_name": "{1}","parameters": [{{"name":"name", "value":"{2}"}}, {{"name":"item_name", "value":"{3}"}}]}}'
#payload={"template_name": "{0}","broadcast_name": "{1}","parameters": [{"name":"name", "value":"{2}"}, {"name":"item_name", "value":"{3}"}]}

send_template_headers = {
  'Authorization': 'Bearer {}'.format(WATI_SERVER_SEND_TOKEN),
  'Content-Type': 'application/json',
  'Cookie': '__cfduid=dc533c9ba4cb8c277f79928e3978a52251619958624'
}

send_template_language_mapping = {'EN':'en_supplier_update_v4', 'HI':'supplier_updates_hi', 'English':'en_supplier_update_v4', 'Hindi':'supplier_updates_hi'}