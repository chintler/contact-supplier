...
def get_data(df):
    global nlp
    columns = ['LANGUAGE','NAME','PINCODE','MATERIAL','AVAILABILITY','DURATION']
...
def process(csv_file_path):
    row_limiter = 262 #enter no. of suppliers  u want
    ...
    cols = ['LANGUAGE','NAME','PINCODE','MATERIAL','AVAILABILITY','DURATION']
    '''
    ...
    req_df = pd.DataFrame(columns=cols)
    suppliers = []
    ...
    for s_ind,supp in enumerate(suppliers):
        ...
        #print('thread')
        #print(thread_df)
        req_df = req_df.append(get_data(thread_df),ignore_index  = True)


    
        #thread_df.to_csv(string)
    #print(req_df)
    req_df.to_csv('req.csv')
    #save it to a csv file
    #supplier_df.to_csv('suppliers_df.csv')
    thread_df.to_csv('threads_ex.csv')
